...
for i in {1..3}; do
        ...
        docker build --tag=manolilop/web${i}-httpd:latest ./PaginaWeb${i}
        docker push manolilop/web${i}-httpd:latest

        # Crear el despliegue
        echo "Creando despliegues..."
        microk8s.kubectl create deployment web${i} --image=manolilop/web${i}-httpd:latest --port=80 --replicas=1
        if [ $? -ne 0 ]; then
        ...
        echo "Exponiendo el despliegues como un servicio NodePort..."
        microk8s.kubectl expose deployment web${i} --type=NodePort --name=web${i}-httpd --cluster-ip=10.152.183.10${i}
        if [ $? -ne 0 ]; then
        ...
        microk8s.kubectl autoscale deployment web${i} --cpu-percent=80 --min=1 --max=3
        if [ $? -ne 0 ]; then 
