...
for i in {1..3}; do
    ...
    microk8s.kubectl create deployment web${i} --image=manolilop/web${i}-httpd:latest --port=80 --replicas=5
    if [ $? -ne 0 ]; then
    ...
    echo "Exponiendo el despliegue como un servicio NodePort..."
    ...
    microk8s.kubectl autoscale deployment web${i} --cpu-percent=$1 --min=$2 --max=$3
    if [ $? -ne 0 ]; then 
